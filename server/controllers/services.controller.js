...
exports.deleteService = async (req, res) => {
	...
	try {
		...
		await findedService.destroy();
		return res.json({
			status: 'ok',
			msg: 'услуга успешно удалена'
		})
	} catch (e) {
		return res.json({
			status: 'error',
			msg: 'не удалось удалить данной услуги'
		});
	}
}
...
exports.createNewService = async (req, res) => {
	...
	try {
		...
		return res.json({
			status: 'error',
			msg: 'не удалось добавить новую услугу'
		})
	}
}
